...
if __name__ == "__main__":
    ...
    with jsonlines.open(args.output_path, mode="w") as outfile:
        for line in dataset:
            # Extract the Arabic premise and hypothesis
            premise_en = line["premise"].get("ar")
            hypothesis_en = line["hypothesis"]["translation"][line["hypothesis"]["language"].index("ar")]

            if not premise_en or not hypothesis_en:
                print("Skipping line due to missing Arabic premise or hypothesis.")
                continue

            # Translate premise
            # first attempt: it included disclaimers about accuracy of translation            
            #premise_prompt = f"Translate Arabic sentence to Moroccan Darija.\n\nArabic: {premise_en}\nDarija:"
            # second attempt: it included output with darija written in latin or Amazigh script
            #premise_prompt = f"Translate Arabic sentence to Moroccan Arabic (Darija). Only respond with the translation output.\n\nArabic: {premise_en}\nDarija:"
            # third attempt: problem (check output 3376179) still yields Note about arabic dialect in Arabic
            #premise_prompt = f"Translate Arabic sentence to the Moroccan Arabic dialect. Only respond with the translation output written in Arabic script.\n\nArabic: {premise_en}\nDarija:"
            # fourth attempt
            premise_prompt = f"Translate the following Arabic sentence to Moroccan Arabic. Only respond with the translated sentence in Arabic letters.\n\nArabic: {premise_en}\nMoroccan Arabic:"
            ...
            hypothesis_prompt = f"Translate the following Arabic sentence to Moroccan Arabic. Only respond with the translated sentence in Arabic letters.\n\nArabic: {hypothesis_en}\nMoroccan Arabic:"
