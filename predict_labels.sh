#!/bin/bash
#SBATCH --job-name=predict_labels         # Job name
...
#SBATCH --output=predict_labels_%j.out   # Main output file
#SBATCH --error=predict_labels_%j.err    # Main error file
...
MODEL_NAMES=("meta-llama/Llama-2-7b-chat-hf") # "meta-llama/Llama-3.1-8B"
LANGUAGES=("en" "ar")
...
for MODEL_NAME_HF in "${MODEL_NAMES[@]}"; do
  for LANGUAGE in "${LANGUAGES[@]}"; do
    ...
    OUTPUT_FILE="${OUTPUT_DIR}/predict_labels_${MODEL_NAME_HF##*/}_${LANGUAGE}.out"
    ERROR_FILE="${OUTPUT_DIR}/predict_labels_${MODEL_NAME_HF##*/}_${LANGUAGE}.err"
